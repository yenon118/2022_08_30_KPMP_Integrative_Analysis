...
create_data_list <- function(dat_list, project, data_dir){
    ...
    mat <- readMM(file = file.path(data_dir, project, "matrix.mtx"))

    feature_names = read.delim(
		file.path(data_dir, project, "features.tsv"), 
		header = FALSE,
		check.names = FALSE,
	...
	barcode_names = read.delim(
		file.path(data_dir, project, "barcodes.tsv"), 
		header = FALSE,
		check.names = FALSE,
		stringsAsFactors = FALSE
	)
    ...
    project <- gsub("1", "", project)

    dat_list[[project]] <- CreateSeuratObject(counts = mat, project = project, min.cells = 8, min.features = 200)
...
for(i in 1:length(directories)) {
  dat_list <- create_data_list(dat_list, directories[i], folder_path)
}
...
directories <- gsub("1", "", directories)
...
for (i in 1:length(dat.list)) {
    dat.list[[i]] <- subset(dat.list[[i]], subset = nFeature_RNA > 100 & nFeature_RNA < 6000 & percent.mt < 90)
}
